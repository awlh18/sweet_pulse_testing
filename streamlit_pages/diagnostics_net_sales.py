...
# model diagnostics
st.title('Regression model diagnostics - Net Sales')
...
X_test = test_df.drop(columns=['net_sales_normalized'])
y_test = test_df['net_sales_normalized']
...
X_train = train_df.drop(columns=['net_sales_normalized'])
y_train = train_df['net_sales_normalized']
...
coef_df = pd.read_csv('results/trained_coef_net_sales.csv')
coef_df[['coefficients']] = coef_df[['coefficients']].astype(int).applymap(lambda x: f"{x:,}")

mae_df = pd.read_csv('results/mae_grouped_net_sales.csv')
mae_df[['net_sales_normalized', 'y_pred', 'prediction_error']] = mae_df[['net_sales_normalized', 'y_pred', 'prediction_error']].astype(int).applymap(lambda x: f"{x:,}")
...
with col_1_2: 
    ...
    column_config={
        'day_of_week': "Day",
        'net_sales_normalized': 'Avg net sales',
