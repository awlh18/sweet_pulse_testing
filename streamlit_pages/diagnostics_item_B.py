...
# model diagnostics
st.title('Regression model diagnostics - Soft serve sales')
...
X_test = test_df.drop(columns=['soft_serve_sales'])
y_test = test_df['soft_serve_sales']
...
X_train = train_df.drop(columns=['soft_serve_sales'])
y_train = train_df['soft_serve_sales']
...
mae_df[['soft_serve_sales', 'y_pred', 'prediction_error']] = mae_df[['soft_serve_sales', 'y_pred', 'prediction_error']].astype(int).applymap(lambda x: f"{x:,}")
...
with col_1_2: 
    ...
    column_config={
        'day_of_week': "Day",
        'soft_serve_sales': 'Avg soft serve sales',
