...
from src.data_validation import _validate_sales_df
...
# upload data 
uploaded_file = st.file_uploader("Data upload", type=['xlsx', 'xls'])
...
def process_sales(uploaded_file):

    sales_df = pd.read_excel(uploaded_file, index_col=0)
...
if uploaded_file is not None:
    sales_df = process_sales(uploaded_file)

if sales_df is None:
    st.warning("Please upload sales data to generate insights.")
    st.stop()
...
sales_A = sales_df['taiyaki_sales'].iloc[-day_range:].sum().astype(int)
sales_B = sales_df['soft_serve_sales'].iloc[-day_range:].sum().astype(int)
sales_C = sales_df['drink_sales'].iloc[-day_range:].sum().astype(int)
...
sales_A_last_period = sales_df['taiyaki_sales'].iloc[-day_range*2:-day_range].sum().astype(int)
sales_B_last_period = sales_df['soft_serve_sales'].iloc[-day_range*2:-day_range].sum().astype(int)
sales_C_last_period = sales_df['drink_sales'].iloc[-day_range*2:-day_range].sum().astype(int)
...
with col_1_2:
    st.metric(label=f"Net sales - Taiyaki", value=f"${sales_A:,}", delta=sales_A_delta_text, border=True) 

with col_1_3:
    st.metric(label=f"Net sales - Soft Serve", value=f"${sales_B:,}", delta=sales_B_delta_text, border=True)

with col_1_4: 
    st.metric(label=f"Net sales - Drinks", value=f"${sales_C:,}", delta=sales_C_delta_text, border=True)
...
core_product_sales = sales_df[['net_sales_normalized', 'taiyaki_sales', 'soft_serve_sales', 'drink_sales']]
